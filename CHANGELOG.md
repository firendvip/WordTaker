...
- 修复 macOS 自动粘贴权限检测误报“已授权”，导致转写结果只进入剪贴板却未进入输入框的问题。
- 权限检测改为校验当前应用身份，并保持非提示式查询，不再主动触发重复授权弹窗。

## [1.29.3] - 2026-08-14

### Fixed

- macOS 仅在当前应用身份已获辅助功能信任时启动裸修饰键/双击取消监听；未授权或检测异常时静默停用，避免应用启动及会话结束后重复触发系统授权窗。
- 保留无需辅助功能权限的组合键与单击 Esc 取消路径，不修改用户现有快捷键设置。

## [1.29.2] - 2026-08-14

### Fixed

- 修复 macOS 默认单击 Esc 因辅助功能监听失败而无法取消录音、隐藏胶囊或小猫的问题。
- Esc 现在可立即取消；识别或非流式润色中的迟到结果不会在取消后继续上屏。

## [1.29.1] - 2026-08-13

### Fixed

- 修复打包版 preload 无法加载导致 Electron API 失效、错误音乐条与原生输入法小猫同时出现的问题。
- 胶囊皮肤读取完成前保持透明，不再闪现未选择的默认音乐皮肤。
