...
- 唤醒喵叫「滞后」：AudioContext 空闲被系统挂起、每次唤醒要先唤醒音频设备导致延迟；加静音保活让音频常驻就绪，唤醒即刻出声
- 唤醒喵叫「音质怪」：去掉唤起音的升调重采样（改回原速播放，消除金属感/压缩噪点被放大）+ 加淡入淡出消除起止轻微爆音

### Added

- 连说 / 尾段再唤醒：结束一段后、胶囊消失前可立即再唤醒开新一段说话，新段马上开录，上一段的转写/润色/粘贴在后台并行收尾、互不干扰——说完一小段紧接下一段，无停顿感（按段隔离，段间出字保序）
- 多猫并存（≤10 只）：连续唤醒可让多只小猫同时在场，每只独立走完录音→转写→润色→粘贴后自行消失；每只独立深色配色（黑优先、同屏不重样、完成即回收）；按唤醒顺序在上一只下方排队（放不下自动改上方）；严格按说话顺序落字（前一只贴完后一只才接着贴，前一只超时最多等 5 秒即跳过）；同屏满 10 只再唤醒会提示"请等前面的说完"

## [1.28.0] - 2026-07-11

### Changed

- 唤醒小猫胶囊改为瞬间出现：去掉入场动画、消除显示延迟（不再等定位卡半拍），与声音同步零延迟
- 唤醒位置默认改为「跟随鼠标」——胶囊出现在鼠标位置附近、瞬间就位、零跳动（不再先出现在别处再跳到输入框）；设置→唤醒位置可切换回「跟随输入框」
- 云端字数不足提醒改为系统弹窗通知（原为托盘图标闪烁），托盘图标不再闪烁；阈值调整为剩余低于 1500 / 500 / 100 时各弹一次

## [1.27.0] - 2026-07-11

### Fixed

- 修复被飞书等会议软件切换系统默认输入设备后、弦外小猫录到无声的问题：录音无信号时明确提示、自动重取一次、设备掉线自动回退默认

### Added

- 设置新增「麦克风」选择：可手动指定输入设备（默认跟随系统），避免被其它应用切换默认设备影响
- 录音诊断日志：记录音轨设备/静音/电平，便于快速定位无声问题

## [1.26.0] - 2026-07-10

### Changed

- 设置「唤醒与结束」快捷键移除三个预设选项，只保留「自定义」按钮（Mac/Win）
- Windows 默认唤醒/结束快捷键改为单击左 Alt（原双击左 Alt）；Mac 保持单击左 Option（快捷键默认仅影响新安装用户，不覆盖老用户已选）
- 默认皮肤改为「小猫」、默认提示音改为「喵」——升级后对所有用户强制生效一次，之后仍可自由更改

## [1.25.1] - 2026-07-10

### Fixed

- 云端额度不足自动降级本地模型润色时，历史记录现如实标注「本地模型」（原误标为「云端AI」）——润色链路回传实际引擎，历史按实际引擎写入，设置项不变（临时降级，额度恢复自动回云端）
- 修复流式润色 IPC 未透传实际引擎导致的同类误标

### Changed

- 本地模型档位描述文案「速度快，质量良好」改为「速度较快，质量良好」

## [1.25.0] - 2026-07-10

### Added

- 本地模型系统提示词改为服务端下发：客户端启动就绪后从后端 `GET /prompt` 拉取完整提示词热注入（`set_prompt`），安装包内不再保留完整提示词明文；拉取失败静默降级为本地精简 fallback，不影响润色可用

### Fixed

- Windows 唤醒后录音框「没说话就自己消失」：透明胶囊窗渲染合成回退不再静默 reload 成空白 + 空录音在唤醒后 800ms 内不立即隐藏（麦克风首帧未就绪守卫）
- Windows 粘贴链路：worker 回执超时 1500→2800ms、sendkeys 超时 1000→1500ms（慢机友好）；三级链全失败时弹系统通知引导手动 Ctrl+V（文本已在剪贴板）
- Windows 客户端上报 `x-platform` 由写死 `mac` 修正为按平台派生（`windows`/`mac`）

### Security

- 本地 fallback 提示词极致精简 + base64·XOR 混淆存储，消灭「解包即得完整提示词明文」；完整提示词真源移至服务端（配合下发接口）
