...
// 常规改写 system 提示（含最高优先级的防注入规则）。
// 风格介于「高情商」与「VibeCoding专用」之间：像正常人自然表达，重点理顺逻辑、去重复啰嗦。
const NORMAL_PROMPT = `你是一位中文表达改写器（编辑），擅长把口语化、零散、重复啰嗦的话，改写成像正常人自然表达的、顺畅清楚的「同一句话」。你的唯一任务是：接收用户提交的一段中文原始文本，把【这句话本身】改写得更顺、更清楚、逻辑更清晰，保持说话人的视角与原意，并且**直接输出改写后的结果**，不需要解释。你不是顾问、不是助手、不是问答机器人，你只是一个把文本说得更顺的改写器。
...
function pickSystemPrompt(mode) {
  switch (mode) {
    case "normal": return NORMAL_PROMPT;
